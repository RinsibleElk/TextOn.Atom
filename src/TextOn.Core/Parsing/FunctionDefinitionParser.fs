...
module internal FunctionDefinitionParser =
    ...
    let parseFunctionDefinition (tokenSet:CategorizedAttributedTokenSet) : ParseError[] * ParsedFunctionDefinition option =
        let (isPrivate, name, tree) =
            if tokenSet.Tokens.Length = 0 then
                ...
                if l < 2 then (false, "<unknown>", (makeParseErrors [|(makeParseError tokenSet.File tokens.[0].LineNumber firstLine.[0].TokenStartLocation firstLine.[0].TokenEndLocation "No name given for function")|]))
                else if l > 4 then
                    let name =
                    ...
                    (false, name, (makeParseErrors [|(makeParseError tokenSet.File tokens.[0].LineNumber firstLine.[0].TokenStartLocation firstLine.[0].TokenEndLocation "Function first line too long")|]))
                else
                    ...
                    | Choice2Of2 errorText ->
                        (false, "<unknown>", (makeParseErrors [|(makeParseError tokenSet.File tokens.[0].LineNumber firstLine.[0].TokenStartLocation firstLine.[0].TokenEndLocation errorText)|]))
                    | Choice1Of2 (isPrivate, name, hasOpenCurly) ->
                        if tokenSet.Tokens.[tokenSet.Tokens.Length - 1].Tokens.Length > 1 || tokenSet.Tokens.[tokenSet.Tokens.Length - 1].Tokens.[0].Token <> CloseCurly then
                            ...
                            (isPrivate, name, (makeParseErrors [|(makeParseError tokenSet.File line.LineNumber attToken.TokenStartLocation attToken.TokenEndLocation "Invalid token")|]))
                        else if not hasOpenCurly then
                            if tokenSet.Tokens.Length < 3 then
                                (isPrivate, name, (makeParseErrors [|(makeParseError tokenSet.File tokens.[0].LineNumber firstLine.[0].TokenStartLocation firstLine.[firstLine.Length - 1].TokenEndLocation "Insufficient brackets")|]))
                            else if tokenSet.Tokens.[1].Tokens.Length > 1 || tokenSet.Tokens.[1].Tokens.[0].Token <> OpenCurly then
                                (isPrivate, name, (makeParseErrors [|(makeParseError tokenSet.File tokens.[1].LineNumber tokens.[1].Tokens.[0].TokenStartLocation tokens.[1].Tokens.[0].TokenEndLocation "Invalid token")|]))
                            else
