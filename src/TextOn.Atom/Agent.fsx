...
let tokenizer = output |> Agent.map (fun s -> s |> Seq.map (fun x -> async { return Tokenizer.tokenize x }) |> Async.Parallel |> Async.RunSynchronously)
...
let start (f:FileInfo) =
    ...
    NewData(file,directory,lines) |> preprocessor.Post
start (FileInfo(@"D:\NodeJs\TextOn.Atom\examples\example.texton"))
output |> Agent.fetch |> Option.get
