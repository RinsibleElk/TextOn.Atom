...
module VariableLineTokenizer =
    let private matches =
        [
            ...
            (Regex("^(\\s*)\\$([A-Za-z][A-Za-z0-9_]*)(\\s*|$)"), (fun n (m:Match) -> { StartIndex = n + m.Groups.[1].Length + 1 ; EndIndex = n + m.Groups.[1].Length + 1 + m.Groups.[2].Length ; Token = VariableName(m.Groups.[2].Value) }))
